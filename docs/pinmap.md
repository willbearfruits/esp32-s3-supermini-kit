...
| 12 | I2C\_SDA | OLED, ToF, IMU SDA |  |
| 13 | I2C\_SCL | OLED, ToF, IMU SCL |  |
| 21 | SPARE | free | ToF XSHUT or IMU INT later |


Pins deliberately left alone: 0 (BOOT button), 19/20 (USB), 43/44 (UART0), 48 (onboard RGB LED), 45/46 (strapping).

Why the joystick sits on GPIO1 and GPIO2: those are ADC1 channels. ADC2 stops working while WiFi is active on the ESP32-S3, so never put analog inputs there.

## Why one I2S port is enough

The ESP32-S3 has two I2S ports. We use only port 0, in full duplex:

- The two DACs are receivers. Receivers can share a bus freely, so both get the same BCLK, WS and data line and play the same stream.

- The mic is a transmitter, so it needs its own data line into the ESP32, but it can still share BCLK and WS with the DACs.

- The only rule is that everyone runs at the same sample rate and slot width. We use 32-bit slots, stereo, Philips format. All three chips accept that.

- None of the three chips needs MCLK.

Total cost: 4 GPIOs for the entire audio path.

Trade-off: both DACs always play the same audio. If you ever want different audio on speaker vs line-out, move one DAC to I2S port 1 (3 more pins).
...
Boards often ship with one or more of these open. An open XSMT means the DAC stays muted forever. This is the one component in the kit that may need a soldering iron, so for a solderless kit either buy pre-bridged boards or move these straps onto the carrier PCB.
