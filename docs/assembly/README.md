...
The friendly ESP32-S3 SuperMini assembly guide

[Download the printable PDF](../../output/pdf/esp32-s3-supermini-assembly-guide.pdf) · [Open the browser checklist](index.html) · [SVG and PNG artwork](assets/)

> On GitHub, download the repository and open `docs/assembly/index.html` locally for the interactive version. No server or internet connection is needed to read it.

## Contents

1. [Make a little noise.](#hello)
2. [Small steps. Happy beeps.](#start)
3. [Seven little teammates.](#parts)
4. [Two rails. One ground.](#power)
5. [Teach it the test tune.](#firmware)
6. [Hello, tiny screen.](#display)
7. [Say hello. Watch it listen.](#microphone)
8. [Meet the purple sound card.](#dac)
9. [Twist. Tilt. Click.](#controls)
10. [Earn your first-sound badge.](#bench)
11. [Print. Measure. Dry-fit.](#case-prep)
12. [Build the shell sandwich.](#case-assembly)
13. [Your tiny band is ready.](#jam)
14. [Add extras after the win.](#extras)
15. [One clue at a time.](#rescue)
16. [Your pocket pin map.](#reference)

<a id="hello"></a>

## 01. Make a little noise.
...
![Illustrated layout of the current case. USB-C and line-out exit the front edge.](assets/hero.svg)

> **YOUR FINISH LINE**
>
> A USB-powered instrument that hears your voice and plays through powered speakers, an audio interface, or a separate headphone amp.
...
> **THE ONE HABIT THAT SAVES BOARDS**
>
> Unplug USB and every other power source before adding, moving, soldering, or checking continuity on a wire. Power up only after checking the new connections.
...
- [ ] I have a data cable and a suitable line-level listening device.
- [ ] I know which parts need soldering or prepared leads.


<a id="parts"></a>

## 03. Seven little teammates.

*01 / MEET THE BAND*

Match the printed labels and the exact board version. Color and shape are clues, never proof of a pinout.

![Simplified identification drawings; not to scale and not a footprint or pin-order reference.](assets/parts.svg)
...
> **LOOK TWICE, PLUG ONCE**
>
> Some OLEDs swap VCC and GND. Some six-pin microphones use another chip. Seller listings are not verified kit specifications. If a pin or board revision is unclear, identify it before powering it.

- [ ] My modules match this list, and their power pins are readable.


<a id="power"></a>

## 04. Two rails. One ground.
...
![USB powers the ESP32. Use its labeled rails only after checking your board's power path.](assets/power.svg)
...
> **UNPLUGGED CHECK**
>
> Check polarity and continuity of each intended ground connection. Check that 3V3-to-GND and 5V-to-GND are not sustained near-zero-ohm shorts. A brief continuity beep from charging capacitors is not a diagnosis; investigate uncertain readings before power-up.
...
- [ ] I can point to 3V3, GND, and the verified USB 5V rail.
- [ ] The DAC VIN rating is confirmed, and no GPIO wire carries 5 V.


<a id="firmware"></a>

## 05. Teach it the test tune.
...
## 06. Hello, tiny screen.

*04 / GIVE IT A FACE*

Disconnect USB. Read the four OLED labels carefully; their order varies between modules.

![Functional connections only. SCL may be printed SCK on an I2C OLED; check the module type.](assets/display.svg)
...
> **SCREEN STILL DARK?**
>
> Unplug first. Check VCC/GND, then SDA/SCL, then the module's I2C address. A seven-pin SPI display or an SH1106 board is not the specified SSD1306 module. If the image is upside down, Kit firmware > Rotate the OLED 180 degrees controls its orientation.
...
- [ ] The screen lights up and the test display updates.


<a id="microphone"></a>

## 07. Say hello. Watch it listen.

*05 / GIVE IT EARS*

Disconnect USB, then add the INMP441. Keep its small sound opening clear of tape, glue, and fingers.

![The mic's SCK is the bit clock on GPIO7. Its SD is data going into GPIO10.](assets/microphone.svg)
...
> **TINY VICTORY**
>
> The board can hear you even before an audio output is connected. If the firmware reports the RIGHT slot, L/R is high; the firmware supports that too. No data means check mic power, SD, SCK and WS with USB unplugged.
...
- [ ] My voice changes the level meter and the mic diagnosis is healthy.


<a id="dac"></a>

## 08. Meet the purple sound card.
...
![The two SCK labels mean different things: mic SCK -> GPIO7; DAC SCK -> GND.](assets/dac.svg)
...
> **THIS JACK IS LINE-OUT**
>
> Connect powered speakers, an audio-interface line input, or a separate headphone amplifier. Ordinary headphones and passive speakers need amplification. TI specifies line loads down to 1 kOhm; a headphone-shaped jack does not make this a headphone amp.
...
- [ ] I hear the startup tone and can hear the mic afterward.
...
![Logical EC11 contacts are shown, not their physical order. Mounting tabs are not signal pins.](assets/controls.svg)
...
> **SAVE THE JOYSTICK VICTORY FOR JAM**
>
> The test app is mainly a mic/DAC/encoder check. Use JAM on page 13 to verify joystick navigation and click. The firmware rotates the joystick axes for the case mounting; do not swap VRx/VRy just to match the drawing's appearance.

- [ ] Encoder turn and push both work; the joystick is powered from 3V3.


<a id="bench"></a>

## 10. Earn your first-sound badge.
...
>
> One failed check is a useful clue. Unplug, inspect that stage, fix it, and repeat the check. The firmware's no shorts between I2S pins message covers only selected signal pairs; it is not a power-rail safety test.
...
- [ ] All the checks above pass with the case open.
- [ ] Both ends of each wire are labeled and the wiring is documented.
- [ ] USB is unplugged before I begin mechanical assembly.


<a id="case-prep"></a>

## 11. Print. Measure. Dry-fit.
...
![Existing OpenSCAD exploded render from this repository. CAD preview, not proof of physical fit.](../../enclosure/out/exploded.png)
...
> **FIT FIRST, HEAT SECOND**
>
> Try every unpowered board in its cradle, including headers, plug bodies and wire bends. Fit may require CAD changes. Install inserts in the empty shell using your insert/filament supplier's method; practice on scrap and let them cool. Check screw length so tips cannot bottom out, pierce plastic, or hit a board.

- [ ] The dry-fit works and every screw is the right length.


<a id="case-assembly"></a>

## 12. Build the shell sandwich.
...
![Conceptual top-view placement, not a drill template. Both panels use the same front-edge reference.](assets/placement.svg)

1. Top shell: fasten the OLED behind its window with four M2 screws. Use light pressure; do not load or pinch the glass.
2. Seat the round mic beside the OLED with its real acoustic port aligned to the grille. Place the gasket around the port; do not cover the opening or force the PCB into the collar.
3. Fit the EC11 through its panel opening, secure the matching washer/nut, and add the knob. Keep enough clearance for the push action. Mount the joystick on its four bosses and check full travel.
4. Lid: seat the ESP32 with USB-C at the front edge. Seat the DAC on the right with its jack facing that same front edge, beside USB. Secure the optional IMU in its own cradle if fitted.
5. Reconnect the labeled harness. Insulate exposed joints, restrain wires, and keep the seam, screw bosses, antenna area and mic port clear. Check connector and pin clearance with the halves gently together.
6. With the case still open on a nonconductive surface, repeat the page 10 sound check. Unplug, align the lip, and close with four lid screws, lightly and evenly. If it resists, reopen and find the obstruction.

- [ ] No wire is pinched; USB/audio plugs seat fully; the knob clicks and stick moves.
- [ ] After closing, the screen, startup tone, microphone and encoder still work.


<a id="jam"></a>

## 13. Your tiny band is ready.
...
> **THE FIRST-BEAT CHALLENGE**
>
> Select DRUMS on PATTERN. Put kicks on steps 1, 5, 9 and 13 of the first bar; add snare on 5 and 13 and hats on odd steps. Repeat in the second bar. Then switch to BASS and add three notes. Hear that? You built the band.
...
- [ ] My beat plays, the joystick works, and my pattern returns after a restart.


<a id="extras"></a>

## 14. Add extras after the win.
...
> **SPEAKER OUTPUT IS NOT GROUND**
>
> Neither speaker terminal goes to GND or to the DAC jack. Do not load the SuperMini's small 3V3 regulator with the amp. For a separate rated amp supply, join grounds but do not tie its positive rail to USB 5V without a designed power path.
...
> **ASK FOR HELP WITH A GOOD CLUE**
>
> Share clear photos of both sides of each board, your GPIO-to-label wiring list, firmware application, and the relevant serial log. Include the exact module revision and what passed before the failure.


<a id="reference"></a>

## 16. Your pocket pin map.
...
> **THE EASY-TO-MISS CONNECTIONS**
>
> Mic L/R -> GND. DAC SCK -> GND. Purple DAC straps: FLT=L, DEMP=L, XSMT=H, FMT=L (confirm your revision). Never put 5 V on GPIO. Leave USB GPIO19/20 and other boot/strapping pins alone.
...
The content lives in `guide.json`; the builder creates all illustrations and formats from it. Requires Python 3.10+, Cairo, and Liberation Sans/Mono fonts (or set `KIT_GUIDE_FONT_DIR` to a directory containing the matching TTF files).

```sh
python3 -m venv /tmp/kit-guide-venv
/tmp/kit-guide-venv/bin/pip install -r docs/assembly/requirements.txt
/tmp/kit-guide-venv/bin/python tools/build-assembly-guide.py
```

The PDF goes to `output/pdf/`. The builder checks the documented GPIO map against `firmware/main/pins.h` and fails on overflowing PDF pages. After edits, render the PDF with `pdftoppm` and visually inspect all pages. Check hardware-specific statements again if firmware or enclosure geometry changes.
