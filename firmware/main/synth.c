...
void synth_init(synth_t *s, const synth_cfg_t *cfg, int voices)
{
    ...
    for (int i = 0; i < s->nv; i++) { s->v[i].note = -1; biquad_lowpass(&s->v[i].lpf, cfg->cutoff, cfg->q); }
}
...
void synth_note_on(synth_t *s, int note, float vel)
{
    ...
    v->stage = 1;
    v->age = ++s->clock;
    if (!legato) { v->ph2 = 0.25f; v->phs = 0; biquad_reset(&v->lpf); }
}
...
void synth_render(synth_t *s, float *out, int n)
{
    ...
    for (int i = 0; i < s->nv; i++) {
        ...
        if (!v->stage) continue;
        float fc = clampf(c->cutoff + c->env_hz * v->env, 60.0f, 7000.0f);
        biquad_lowpass(&v->lpf, fc, c->q);
        float g = c->gain * v->vel;
        for (int k = 0; k < n; k++) {
            switch (v->stage) {
            ...
            if (!v->stage) break;
            onepole(&v->freq, v->freq_t, v->glide_c);
            float dt = v->freq / FS;
            ...
            out[k] += biquad_run(&v->lpf, y) * v->env * g;
        }
    }
}
