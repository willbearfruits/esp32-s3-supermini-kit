...
typedef struct { bool on; float pos, rate, amp; } svoice_t;
...
static struct {
    ...
    int   sel; bool perform, mute[CH_N]; float mix[CH_N][MX_N];
...
// ---- voices ----------------------------------------------------------------
static void sample_on(float rate, float vel)
{
    if (S.smp_len <= 0) return;
    svoice_t *v = &S.sv[S.sv_rr]; S.sv_rr = (S.sv_rr + 1) % SV_N;
    v->on = true; v->pos = 0; v->rate = rate; v->amp = vel;
}

static void sample_render(float *out, int n)
{
    if (S.perform && S.sel == CH_SAMPLE && S.smp_len > 1) {
    ...
    for (int k = 0; k < SV_N; k++) {
        ...
        for (int i = 0; i < n; i++) {
            int p = (int)v->pos;
            if (p + 1 >= S.smp_len) { v->on = false; break; }
            float f = v->pos - p;
...
static void fire_step(int s)
{
    ...
    if ((r = S.note[lane(CH_SAMPLE)][s]) >= 0 && !S.mute[CH_SAMPLE] && !(S.perform && S.sel == CH_SAMPLE))
        sample_on(powf(2.0f, row_semi(r) / 12.0f), 0.9f);
}
...
static void do_cmd(const cmd_t *c)
{
    ...
    case CMD_PERFORM:   S.perform = c->a; if (S.perform && S.sel == CH_SAMPLE) { S.scratch_rate = 0; } break;
    }
}
...
void jam_get_ui(jam_ui_t *u)
{
    ...
    if (S.sel == CH_DRUMS) {
        ...
        for (int r = 0; r < u->rows; r++) {
            ...
            u->root_row[r] = row % s->n == 0;
            u->row_label[r][0] = (char)('1' + row % s->n); u->row_label[r][1] = 0;
            for (int st_ = 0; st_ < STEPS; st_++) u->grid[r][st_] = S.note[l][st_] == row;
        }
    }
